...
Notebook **00** copies a separate lab Parquet from the Git folder working
copy (gitignored; not on GitHub). Columns and path:
...
`rm`s `external-tables/fare_dv_lab`, then copies the repo Parquet. `DROP
TABLE` does not delete those files. Module 5 `99` Level 1 does not clear
...
| # | Notebook | Focus |
|---|---|---|
| 00 | Copy Fare DV Lab File | Utility. Open from the course Git folder. `DROP TABLE IF EXISTS fare_dv_lab`; `rm` the destination folder; copy; `LIST`. Do not `CREATE TABLE`. Fence: no DV teaching, `UPDATE`, `VACUUM`, `OPTIMIZE`. **No exercise**. **Next:** `01 - Deletion Vectors, REORG TABLE, and VACUUM` |
