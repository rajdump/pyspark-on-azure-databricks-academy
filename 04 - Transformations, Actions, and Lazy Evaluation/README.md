...
| # | Notebook | Focus |
|---|---|---|
| 01 | Transformations vs Actions | Transformations vs actions; chain before one action — example 1 (`filter`, `withColumn`, `select`); example 2 (`filter`, `orderBy`, `limit`, `select`) |
| 02 | Lazy Evaluation and the Query Plan | Why Spark waits for an action; `.explain(mode="extended")`; optimizer can push a late filter earlier on one narrow chain |
...
| 04 | Common DataFrame Actions | Return types and driver size risk (`show` / `count` / `collect` already known); sort then compare `first()` / `head()` / `head(n)` / `take(n)`; `tail(n)` (order not guaranteed unless sorted); `isEmpty()` vs `count() == 0`; `toPandas()` same driver risk as `collect()`; `DataFrame.write` → Module 5 |
