...
| 04 | Delta Time Travel and Restore | Self-contained managed `fare_timetravel_lab` (no `LOCATION`, DV off). Build history then identify versions from `DESCRIBE HISTORY` (no Python version captures): CREATE (0) → INSERT 1001–1003 (1, 3 rows, 1003 tip **6.00**) → INSERT 1004 (2, 4 rows) → UPDATE 1003 → **10.00** (3, 4 rows, 1002 present) → DELETE 1002 (4, 3 rows). After DELETE: leftover files from those `INSERT`/`UPDATE`/`DELETE` commits are why later `VERSION AS OF` still works (no `LIST`; managed). HISTORY (`version`, `timestamp`, `operation`; SQL `INSERT`s typically `WRITE`). Time travel: `VERSION AS OF 2` vs current unchanged, then `VERSION AS OF 3`; `TIMESTAMP AS OF` version 2's HISTORY timestamp (at-or-before; same as version 2); one PySpark `versionAsOf` **2**. Time travel (read) vs `RESTORE` (new commit; can reuse leftover files still on disk). One executable `RESTORE TO VERSION AS OF 3` (1002 back, 1003 = **10.00**, HISTORY version 5); timestamp `RESTORE` syntax only. If cells are re-run, use HISTORY numbers from that run. Retention: leftover files from this notebook's writes; 30-day history vs 7-day `VACUUM` eligibility; do not run `VACUUM`. After the exercise: next-problem story (changing data vs physically removing old files; name deletion vectors / `REORG` / `VACUUM` as the cleanup sequence only — do not teach them). Fence: no `CLONE`, CDF, JSON, `@v`, teaching-table time travel, retention `ALTER`, PySpark `timestampAsOf` / `restoreToVersion`, `exceptAll`, `latest_history` helper, DV/`REORG`/`VACUUM` behavior. Exercise: PySpark `versionAsOf` **4** after restore (**3** rows); current still **4**. Module ends here; narrative next is Module 11 `01` (logical change vs physical cleanup) |
