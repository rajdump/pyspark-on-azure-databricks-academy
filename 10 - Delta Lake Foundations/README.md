...
| 04 | Delta Time Travel and Restore | Self-contained managed `fare_timetravel_lab` (no `LOCATION`, DV off). Build history: CREATE (0) → INSERT 1001–1003 (3, 1003 tip **6.00**) → INSERT 1004 (4) → UPDATE 1003 → **10.00** (4) → DELETE 1002 (3). `DESCRIBE HISTORY` (`version`, `timestamp`, `operation`; SQL `INSERT`s typically `WRITE`). Time travel: `VERSION AS OF` before-update vs current unchanged, then after-update as restore target; `TIMESTAMP AS OF` (at-or-before, same before-update snapshot); one PySpark `versionAsOf`. Time travel (read) vs `RESTORE` (new commit). One executable `RESTORE TO VERSION AS OF` the UPDATE snapshot (1002 back, 1003 = **10.00**); timestamp `RESTORE` syntax only. Capture versions from history — do not hardcode `VERSION AS OF 2`. Retention: not a backup; 30-day history vs 7-day `VACUUM` eligibility; do not run `VACUUM`. Fence: no `CLONE`, CDF, JSON, `@v`, teaching-table time travel, retention `ALTER`, PySpark `timestampAsOf` / `restoreToVersion`. Exercise: `versionAsOf` `restore_version` vs `delete_version`, then `exceptAll` for the row that disappeared (**1** row, trip **1002**); do not `RESTORE`. Module ends here; next is Module 11 |
