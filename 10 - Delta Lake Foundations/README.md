...
This module is the lakehouse **storage foundation** later modules depend on.
It does not teach ACID internals, concurrency, schema evolution, deletion
vectors, `MERGE`, or `OPTIMIZE` / `VACUUM` behavior — those are Module 11.
Grants are Module 12.
...
| Asset | Notes | Source |
|---|---|---|
| Catalog `rideshare_dev`; schemas `landing`, `processed` | Course catalog | Module 5 `01 - Unity Catalog Volumes and Data Landing.py` |
...
(Module 10 — Delta Lake labs). Columns from `trip.service_type` and
`payment` (no `driver_payout_amount`). `service_type` uppercase;
