...
| 03 | Managed vs External Delta Tables | Self-contained original four rows (1003 stays **6.00**; 1001 stays **3.00**). Empty managed `CREATE` (no `LOCATION`, DV off) + empty external `CREATE … LOCATION` at `{url}/external-tables/fare_external_lab` (**0** rows) → `INSERT` both (**4**) → type/location proof (`DESCRIBE DETAIL` format+location; `rideshare_dev.information_schema.tables` `table_type`/`storage_path`; `LIST` external succeeds, `LIST` managed URI expected failure) → `DROP` both / `SHOW TABLES DROPPED` / external files remain → `UNDROP` both (**4**; managed = relation + files UC retained; external = relation over files that never left) → leave managed undropped; external `DROP` + re-register `CREATE … LOCATION` without column list (**4**) → decision guide (landing Volume / Bronze-Silver-Gold managed / fixed-path external; managed default = Databricks-managed storage and optimizations). Fence: no `UPDATE`, `DESCRIBE HISTORY`, `OPTIMIZE`, `VACUUM`, `VERSION AS OF`/`RESTORE`; no `GRANT` (Module 12); no Volume `LOCATION`; no Predictive Optimization demo. **No exercise** |
| 04 | Delta Time Travel and Restore | Self-contained managed `fare_timetravel_lab` (no `LOCATION`, DV off). `spark.sql` with `lab_table`; no `%sql` CREATE. Semantic captures only: `before_update_version` / `before_update_timestamp` after second `INSERT`; `restore_version` after `UPDATE`; `delete_version` after `DELETE`; `after_version_restore` after `RESTORE`. Never hardcode `VERSION AS OF 2`. `v0`–`v4` expected-history timeline in one markdown cell only. `CREATE` (**0**) → `INSERT` 1001–1003 (**3**, 1003 = **6.00**, print only) → `INSERT` 1004 (**4**, `display`) → one `sleep(2)` → `UPDATE` 1003 → **10.00** (**4**, 1002 present) → `DELETE` 1002 (**3**). `DESCRIBE HISTORY` is the index (`version`, `timestamp`, `operation`; SQL `INSERT`s typically show as `WRITE`). `VERSION AS OF` before-update then current unchanged; compare by querying `restore_version` only. `TIMESTAMP AS OF` before-update timestamp (at-or-before; same snapshot). One PySpark `versionAsOf` (`timestampAsOf` supported, omitted). Time travel vs `RESTORE`; one executable `RESTORE TO VERSION AS OF` the UPDATE version (**4** rows, 1002 back, 1003 = **10.00**, new HISTORY row; do not display restore metrics). Timestamp `RESTORE` syntax in markdown only (do not run). Retention: not a backup; 30-day history vs 7-day `VACUUM` eligibility; do not run `VACUUM`. Title cell: one sentence — does not cover Delta maintenance or advanced change-tracking. Fence (Focus only): no `UNDROP`, `CLONE`, CDF, JSON, `@v`, time travel on teaching tables, `VACUUM` command, retention `ALTER`, Predictive Optimization, PySpark `timestampAsOf` / `restoreToVersion`, `exceptAll`. Exercise: `VERSION AS OF` `delete_version` after restore (**3** rows, 1002 gone); current still **4**; do **not** `RESTORE`. Module 10 ends here; next is Module 11 |
