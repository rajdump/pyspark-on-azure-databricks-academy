...
| 04 | Delta Time Travel and Restore | Self-contained managed `fare_timetravel_lab` (no `LOCATION`; no `delta.enableDeletionVectors` property). Build five versions: CREATE → INSERT 1001–1003 → INSERT 1004 → UPDATE 1003 tip **10.00** → DELETE 1002. `%sql` `SELECT version, timestamp, operation FROM (DESCRIBE HISTORY …)` on the fixed table name (no Python version captures). Time travel: `VERSION AS OF 2` vs current unchanged. Executable `%sql` `TIMESTAMP AS OF` with a literal timestamp (not a HISTORY lookup). One PySpark `versionAsOf` **2**. `RESTORE TO VERSION AS OF 3` (1002 back, tip **10.00**; HISTORY shows `RESTORE` as a new version). Timestamp `RESTORE` syntax only. Retention: 30-day history vs 7-day `VACUUM` eligibility; files stay until `VACUUM`; `VERSION AS OF` can fail after those files are gone; do not run `VACUUM`. Fence: no `CLONE`, CDF, JSON, `@v`, teaching-table time travel, retention `ALTER`, PySpark `timestampAsOf` / `restoreToVersion`, `exceptAll`, `latest_history` helper, `VERSION AS OF 3` demo, leftover-file inventory, DV/`REORG` teaching. Exercise: PySpark `versionAsOf` **4** after restore (**3** rows); current still **4**. **Next:** Module 11 `01 - Deletion Vectors, REORG TABLE, and VACUUM` (cleanup of obsolete data and its effect on historical access — do not teach how cleanup works) |

## Notebook 04 fence

Notebook **04** teaches table history, time travel by version and by
timestamp, PySpark `versionAsOf`, `RESTORE`, and retention. It does not
run `VACUUM`. It does not set `delta.enableDeletionVectors`. It does not
teach deletion vectors, `REORG TABLE`, leftover-file rewrite behavior, or
how cleanup works.

**Next** names Module 11 `01 - Deletion Vectors, REORG TABLE, and VACUUM`
as cleanup of obsolete data and its effect on historical access.
