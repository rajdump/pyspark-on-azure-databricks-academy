...
| 04 | Delta Time Travel and Restore | Self-contained managed `fare_timetravel_lab` (no `LOCATION`; do not set `delta.enableDeletionVectors`). Build history then identify versions from `DESCRIBE HISTORY` (no Python version captures): CREATE (0) → INSERT 1001–1003 (1, 3 rows, 1003 tip **6.00**) → INSERT 1004 (2, 4 rows) → UPDATE 1003 → **10.00** (3, 4 rows, 1002 present) → DELETE 1002 (4, 3 rows). HISTORY (`version`, `timestamp`, `operation`; SQL `INSERT`s typically `WRITE`). Time travel: `VERSION AS OF 2` vs current unchanged, then `VERSION AS OF 3`; `TIMESTAMP AS OF` syntax only (literal `'<timestamp>'`; at-or-before; HISTORY timestamp for version 2 is the same state as `VERSION AS OF 2`; no Python HISTORY lookup); one PySpark `versionAsOf` **2**. Time travel (read) vs `RESTORE` (new commit; can reuse files from the restored version). One executable `RESTORE TO VERSION AS OF 3` (1002 back, 1003 = **10.00**, HISTORY version 5); timestamp `RESTORE` syntax only. Retention: 30-day history vs 7-day `VACUUM` eligibility; `VACUUM` removes eligible old data files, which can limit historical access; do not run `VACUUM`. Fence: no `CLONE`, CDF, JSON, `@v`, teaching-table time travel, retention `ALTER`, PySpark `timestampAsOf` / `restoreToVersion`, `exceptAll`, `latest_history` helper. Do **not** explain physical file rewrite, files added/removed by `INSERT`/`UPDATE`/`DELETE`, deletion vectors, `REORG`, or `VACUUM` mechanics. No DV/`REORG`/`VACUUM` diagrams. Exercise: PySpark `versionAsOf` **4** after restore (**3** rows); current still **4**. Module ends after the exercise and summary. **Next** may only name Module 11 `01 - Deletion Vectors, REORG TABLE, and VACUUM` as cleanup of obsolete data — it must not explain how that cleanup works |

## Notebook 04 fence

Notebook **04** teaches only table history, time travel, `RESTORE`, and
the retention concept required to understand historical access.

**Next** names Module 11 `01 - Deletion Vectors, REORG TABLE, and VACUUM`
as cleanup of obsolete data. It must not explain how that cleanup works.

Do not explain physical file rewrite behavior, or which files
`INSERT` / `UPDATE` / `DELETE` add or leave behind (notebook **02**). Do
not teach deletion vectors, `REORG TABLE`, or `VACUUM` beyond this
retention statement: `VACUUM` removes eligible old data files, which can
limit access to historical versions. Do not run `VACUUM`. Do not set
`delta.enableDeletionVectors`. Do not include diagrams of deletion
vectors → `REORG` → `VACUUM`.
