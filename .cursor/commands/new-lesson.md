...
2. Determine the next notebook number within that module folder by
   ...
   exist).
3. Ask for (or infer from context) a short, descriptive Title Case name for
   the notebook, following the naming rules in `naming-conventions.md`.
4. Create the file as `NN - Title.py` inside the module folder, using the
...
5. Populate it with a skeleton that follows `teaching-guidelines.md`
   (objectives first, worked example before exercise placeholder) — not
   full lesson content unless explicitly asked to write it now.
6. If the notebook will use the shared dataset, reference the correct
