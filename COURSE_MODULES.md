...
Build Delta Lake foundations, then schema, maintenance, and introductory
`MERGE`; govern existing assets; design the medallion architecture; and
...
| 11 | Delta Lake Transactions, Schema, and Maintenance | Apply schema change, introductory `MERGE`, deletion-vector maintenance, and ACID / optimistic concurrency after the foundations module | Schema enforcement and evolution; introductory `MERGE`; deletion vectors; `REORG TABLE ... APPLY (PURGE)`; `VACUUM`; ACID and optimistic concurrency | Module 10 | Transactional correctness, table maintenance, and upsert syntax used before incremental pipelines | Delta operations later reused on medallion tables; production `MERGE` is Module 15 | Not Started |
