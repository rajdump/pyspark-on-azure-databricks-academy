...
| # | Module | Purpose | Major Topics | Prerequisites | Production Relevance | Final-Project Contribution | Status |
|---|---|---|---|---|---|---|---|
| 5 | Reading, Writing, and Schemas | Land rideshare files on UC Volumes and read/write production formats with explicit schemas (hybrid I/O + minimal reshape; systematic transforms deferred to Module 6) | UC catalog/schema/volume awareness; `CREATE VOLUME` under existing `el_lab`; repo → Volume upload to `raw/{dataset}/`; Azure SQL JDBC write/read (`el_lab.payments`, secrets `el-lab`/`sql-password`) and Avro to `raw/payment/` (all-purpose cluster); one-format-per-dataset reads — CSV (`trip`), JSON (`zone_lookup`), Parquet (`trip_time`), XML (`drivers`, `rowTag` only), Avro (`payment`); explicit schema vs `inferSchema`; format trade-offs; light rename/select/basic cast; save modes; brief partitioned writes; Delta file-format preview and one `saveAsTable` preview (deep Delta → Module 10; UC grants/external-location design → Module 11) | Module 4 | Volume-based file I/O and JDBC extraction patterns used in real ingestion jobs | Builds the Volume `raw`/`processed`/`source` ingestion layer for the final project | Not Started |
| 6 | Built-in Functions, Complex Types, and UDF Alternatives | Implement business transforms with Spark-native expressions before reaching for UDFs | `pyspark.sql.functions` reference (string/date/numeric/null/conditional; callback to Module 3 overlaps); applied multi-step chains on all datasets; structs/arrays/maps; `explode`/`posexplode`; higher-order `transform`/`filter`/`exists`; write cleaned `processed/{dataset}/` (e.g. `drivers_flat` for Module 7 handoff); built-in vs Python UDF framework; Pandas UDFs / `mapInPandas` (grouped `applyInPandas` → Module 8) | Module 5 | Performant, idiomatic transformation logic without unnecessary UDFs | Implements cleaning, enrichment, and business-logic transforms | Not Started |
| 7 | Joins and Set Operations | Combine rideshare tables without silent cardinality or key traps | Inner/left/right/full joins; all four semi/anti variants; join correctness (cardinality, row multiplication, null keys, ambiguous columns, validation); multiple joins and `zone_lookup` pickup/dropoff pattern; cross/inequality/AsOf-range on synthetic timestamps; broadcast joins, AQE, strategy hints; `union`/`unionByName`/`intersect`/`except`/`distinct`/`dropDuplicates`; drivers→trip join after Module 6 flatten (callback only) | Module 6 | Multi-table integration — a core production pattern | Combines `trip`/`payment`/`zone_lookup` (and flattened `drivers`) into unified views | Not Started |
| 8 | Aggregations and Window Functions | Produce analytics-ready summaries and KPI tables | `groupBy().agg()` (callback: Module 4 used shuffle demos; here for analytics); advanced aggs (`collect_list`/`collect_set`, conditional); pivot/reshape; window specs, ranking, frames, lag/lead, running totals; Top-N per group; sorting; short `sample`/`sampleBy`; short `applyInPandas` awareness (prefer built-ins) | Module 7 | Analytics and reporting layers | Produces KPI/metric tables (`trip_enriched`, etc.) for the project | Not Started |
| 9 | Spark SQL and DataFrame Interoperability | Express and validate pipeline logic in both DataFrame and SQL APIs (dual-cell default in this module) | Same engine; temp views; light `EXPLAIN`; joins and set ops in SQL (side-by-side with Module 7); aggs/windows/frames and `QUALIFY`; CTEs/subqueries/params/timezone and `LATERAL VIEW explode` (dual with Module 6); end-to-end pipeline using Modules 5–8 skills; optional SQL CTAS table preview (not medallion/Delta-deep/UC-grants) | Module 8 | Supports SQL-first team collaboration and dual-API validation | Enables SQL-based transforms and tests for the project | Not Started |
