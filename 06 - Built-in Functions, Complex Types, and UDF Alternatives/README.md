...
| # | Notebook | Focus |
|---|---|---|
| 01 | Column Transforms with Built-in Functions | Volume vs managed table on **`trip_time`**: landing **`/Volumes/rideshare_dev/landing/source_files/trip_time/trip_time.parquet`** vs **`rideshare_dev.processed.trip_time_preview`** — same transforms after load; built-ins by dataset (**`trip`**: string, numeric/decimal, conditional — no date cols; **`trip_time`**: **`trip_date`**, **`hour_of_day`**; optional light **`payment`** decimals); no curated write — **`03`** re-reads landing |
| 02 | Complex Types, Structs, Arrays, and explode | Landing **`drivers`** XML with **`rowTag`** (same as Module 5 `05 - Reading XML.py` — do not read `/Volumes/rideshare_dev/processed/output_files/practice/`); struct fields (**`vehicle.*`**); arrays; **`explode`** / **`explode_outer`** on **`trips_assigned`**; write **`/Volumes/rideshare_dev/processed/output_files/curated/drivers_flat/`** |
| 03 | Cleaning and Curated Outputs | Full-size **`bad_trip_data.csv`** (108 source rows) and **`bad_payment_data.csv`** (106 source rows); one forward chain per file (missing-key rejection, **`dropDuplicates`** on trip **`trip_id`**, normalization, failed-conversion handling, invalid-value rules); persist enrichment/cleaning columns only here; write **`/Volumes/rideshare_dev/processed/output_files/curated/trip/`** and **`/Volumes/rideshare_dev/processed/output_files/curated/payment/`** |
| 04 | Built-ins First, When (Not) to Use UDFs | Built-ins as default; Python UDF contrast; short Pandas/Arrow note (not taught further); demo on a small column rule — **do not overwrite** curated outputs |
